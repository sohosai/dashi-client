...
const IndividualItemResult: FC<Props> = (props) => {
  return (
    <>
      ...
      <p>created_at: {timestampConverter(props.result.created_at)}</p>
      <p>updated_at: {timestampConverter(props.result.updated_at)}</p>
      <p>recipient: {props.result.recipient}</p>
      <p>rental_description: {props.result.rental_description}</p>
      <p>latest_rent_at: {timestampConverter(props.result.latest_rent_at)}</p>
      <p>scheduled_replace_at: {scheduledReplaceAtConverter(props.result.scheduled_replace_at)}</p>
      <p>latest_replace_at: {timestampConverter(props.result.latest_replace_at)}</p>
    </>
  );
};
